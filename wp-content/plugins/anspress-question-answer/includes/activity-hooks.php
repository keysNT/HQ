...
class AnsPress_Activity_Hook
{
	...
	public function __construct() {
		...
		anspress()->add_action( 'ap_after_save_question', $this, 'send_all_notification' );
		anspress()->add_action( 'ap_after_new_answer', $this, 'new_answer' );//send_all_notification
	...
	public function new_question($question_id) {
		...
		$activity_arr = array(
			...
			'content'			=> sprintf( __( '%s asked post %s', 'anspress-question-answer' ), ap_activity_user_name( $question->post_author ), $question_title ),
			'term_ids'			=> $term_ids,
		);
	...
	public function edit_question($post_id) {
		...
		$activity_arr = array(
			...
			'content' 			=> sprintf( __( '%s edited post %s', 'anspress-question-answer' ), ap_activity_user_name( get_current_user_id() ), $question_title ),
		);
	...
	/**
	 * Send notification when someone share question
	 */
	public function send_all_notification($post_id){
		error_log('post_id');
		error_log($post_id);
		$is_share_all = get_post_meta($post_id,'is_share');
		error_log('is_share');
		error_log($is_share_all);
		if($is_share_all){
			$question = get_post($post_id);
			$question_title = '<a class="ap-q-link" href="'. wp_get_shortlink( $post_id ) .'">'. get_the_title( $post_id ) .'</a>';
			$activity_arr = array(
				...
				'status'			=> $question->post_status,
				'permalink' 		=> wp_get_shortlink( $post_id ),
				'content' 			=> sprintf( __( '%s created post %s', 'anspress-question-answer' ), ap_activity_user_name( get_current_user_id() ), $question_title ),
			);
			$activity_id = ap_new_activity( $activity_arr );

			// Add question activity meta.
			ap_update_post_activity_meta( $post_id, 'create_post', get_current_user_id() );
			// Notify users
			global $wpdb;
			...
			foreach ($results as $result){
				$role = get_post_meta($result['ID'], 'wp_user_level');
				$subscribers[] = $result['ID'];
			}
			// Remove current user from subscribers.
			if ( $activity_id ) {
				$subscribers = ap_unset_current_user_from_subscribers( $subscribers );
