...
function ap_get_ask_form_fields( $post_id = false ) {
	...
	//add check box share with all
	$fields[] = array(
		'name' => 'is_share_all',
		'type'  => 'checkbox',
		'desc'  => __( 'Share with all.', 'anspress-question-answer' ),
		'value' => $is_share_all,
		'show_desc_tip' => false,
		'sanitize' => array( 'only_boolean' ),
	);
...
function ap_ask_form( $editing = false ) {
	...
	$args = array(
		...
		'is_ajaxified'      => true,
		'multipart'         => true,
		'submit_button'     => ($editing ? __( 'Update question', 'anspress-question-answer' ) : __( 'Share', 'anspress-question-answer' )),
...
function ap_save_question($args, $wp_error = false) {
	...
	$is_share = isset($args['is_share_all'])?$args['is_share_all']:'';
	...
	update_post_meta( $post_id, 'is_share', $is_share);
	error_log('id post');
	error_log($post_id);
	error_log($is_share);
