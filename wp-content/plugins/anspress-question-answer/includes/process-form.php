...
class AnsPress_Process_Form
{
	...
	public function process_ask_form() {
		...
		if ( isset( $fields['is_share_all'] ) ) {
			$question_array['is_share_all'] = $fields['is_share_all'];
		}

		$post_id = ap_save_question( $question_array, true );
		do_action( 'ap_after_save_question', $post_id);

		//update_post_meta($post_id,'share_all',$is_share_all);
		if ( $post_id ) {
			ap_ajax_json( array(
				'action' 		=> 'new_question',
	...
	public function edit_question() {
		...
		if ( $this->fields['is_share_all'] ) {
			$question_array['is_share_all'] = $this->fields['is_share_all'];
		}
