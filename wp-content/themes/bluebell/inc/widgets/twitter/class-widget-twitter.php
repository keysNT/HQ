...
if ( defined( 'FW' ) && function_exists( 'fw_ext_social_twitter_get_connection' ) && function_exists( 'curl_exec' ) ) {
	...
	class Topseo_Widget_Twitter extends WP_Widget {
		...
		function __construct() {
			...
			parent::__construct( false, esc_html__( 'Twitter', 'consult' ), $widget_ops );
		}
		...
		function form( $instance ) {
			...
			<p>
				<label for="<?php echo esc_attr($this->get_field_id( 'user' )); ?>"><?php esc_html_e( 'User', 'consult' ); ?> :</label>
			...
			<p>
				<label for="<?php echo esc_attr($this->get_field_id( 'number' )); ?>"><?php esc_html_e( 'Number of tweets', 'consult' ); ?>
					:</label>
